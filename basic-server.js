...
async function processAudio(audioData, connectionId) {
  ...
  try {
    ...
    await delay(500); // 500ms delay for STT
...
async function generateResponse(text, connectionId) {
  ...
  try {
    ...
    await delay(1000); // 1 second for LLM processing
...
const PORT = 3000;
server.listen(PORT, () => {
  console.log(`Basic Emma AI server running on http://localhost:${PORT}`);
});
